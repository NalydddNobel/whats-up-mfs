...
namespace Polarities.NPCs.Eclipxie.EclipxieRework
{
    [AutoloadBossHead]
    public class Rework_Eclipxie : ModNPC
    {
        ...
        {
            NPC pixie = Main.npc[NPC.NewNPC(NPC.GetBossSpawnSource(player.whoAmI), (int)player.Center.X, (int)player.Center.Y - 300, NPCType<Rework_Eclipxie>())];
            ...
            switch (NPC.ai[0])
            {
                ...
                case -1:
                    ...
                    if (NPC.ai[1] == 119)
                    {
                        ...
                        sunMinion = NPC.NewNPC(NPC.GetSource_FromAI(), (int)NPC.Center.X, (int)NPC.Center.Y, NPCType<Rework_SolMoth>(), ai0: NPC.whoAmI);
                        moonMinion = NPC.NewNPC(NPC.GetSource_FromAI(), (int)NPC.Center.X, (int)NPC.Center.Y, NPCType<Rework_LunaButterfly>(), ai0: NPC.whoAmI);
                ...
                case 2:
                    {
                        ...
                        else
                        {
                            ...
                            if ((NPC.ai[1] - setupTime) % attackPeriod == 0)
                            {
                                ...
                                for (int i = -10; i <= 10; i++)
                                {
                                    Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center + projectileOffset * i, projectileVelocity, ProjectileType<Rework_EclipxieRay>(), ProjectileDamage, 0f, Main.myPlayer, ai0: i, ai1: parity);
                                }
                            }
                        }
                ...
                case 3:
                    {
                        ...
                        else
                        {
                            ...
                            if (NPC.ai[1] == setupTime + telegraphTime)
                            {
                                //produce planets
                                for (int i = 0; i < 300; i++)
                                    Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center, Vector2.Zero, ProjectileType<Rework_EclipxieOrbiter>(), ProjectileDamage, 0, Main.myPlayer, ai0: NPC.whoAmI, ai1: i);
                                //produce deathrays
                                Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center, (player.Center - NPC.Center).SafeNormalize(Vector2.Zero).RotatedBy(MathHelper.Pi / 6), ProjectileType<Rework_EclipxieRaysBig>(), ProjectileDamage, 0f, Main.myPlayer, ai0: NPC.whoAmI, ai1: 0);
                ...
                case 4:
                    {
                        ...
                        else if (NPC.ai[1] < setupTime + attackTime)
                        {
                            ...
                            {
                                if (NPC.localAI[0] != 1)
                                    Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center, new Vector2(1, 0).RotatedBy((player.Center - NPC.Center).ToRotation()), ProjectileType<Rework_EclipxieRayStar>(), ProjectileDamage, 0f, Main.myPlayer, ai1: 0);
                                if (NPC.localAI[0] != 0)
                                    Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center, new Vector2(1, 0), ProjectileType<Rework_EclipxieRayStar>(), ProjectileDamage, 0f, Main.myPlayer, ai1: 1);
                            }
                        }
                        else
                        {
                        ...
                        else
                        {
                            ...
                            if ((NPC.ai[1] - setupTime) % 180 == 0 && NPC.ai[1] - setupTime < 360)
                            {
                                ...
                                for (int i = 0; i < 10; i++)
                                {
                                    ...
                                    Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center, new Vector2(32 * speedMult, 0).RotatedBy((player.Center - NPC.Center).ToRotation() + (i + 0.5f) * MathHelper.TwoPi / 10), ProjectileType<Rework_EclipxieRayStar>(), ProjectileDamage, 0f, Main.myPlayer, ai0: i == 0 ? 1.5f : 1, ai1: (i + (parity ? 1 : 0)) % 2);
                                }
                            }
                        }
                        ...
                        else if (NPC.ai[1] < setupTime + attackTime)
                        {
                            ...
                            if (NPC.ai[1] < setupTime + attackTime)
                                for (int side = -1; side <= 1; side += 2)
                                {
                                    ...
                                    Projectile.NewProjectile(NPC.GetSource_FromAI(), shotPosition, shotVelocity, ProjectileType<Rework_EclipxieMeteor>(), ProjectileDamage, 0f, Main.myPlayer, ai1: 1);
                                }
                            if ((NPC.ai[1] - 20 - setupTime) % 60 == 0 && NPC.ai[1] < setupTime + attackTime - 120)
                            {
                                ...
                                for (int i = 0; i < 3; i++)
                                {
                                    Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center, new Vector2(projSpeed, 0).RotatedBy(i * MathHelper.TwoPi / 3 + rotOffset), ProjectileType<Rework_EclipxieMeteor>(), ProjectileDamage, 0f, Main.myPlayer, ai0: 3, ai1: 0);
                                }
                            }
                        }
                        else
                ...
                case 7:
                    {
                        ...
                        else if (NPC.ai[1] < setupTime + attackPart1Time)
                        {
                            ...
                            while (NPC.ai[2] >= 1)
                            {
                                ...
                                for (int i = 0; i < 2; i++)
                                {
                                    int projIndex = Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center, new Vector2(Main.rand.NextFloat(10), 0).RotatedByRandom(MathHelper.TwoPi), ProjectileType<Rework_EclipxieMeteor>(), ProjectileDamage, 0f, Main.myPlayer, ai0: 1, ai1: i);
                ...
                case 8:
                    {
                        ...
                        else if (NPC.ai[1] < setupTime + hoverTime)
                        {
                            if (NPC.ai[1] == setupTime)
                            {
                                Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center, new Vector2(0, 1), ProjectileType<Rework_EclipxieRaysBig>(), ProjectileDamage, 0f, Main.myPlayer, ai0: NPC.whoAmI, ai1: 2);
                            }
                        ...
                        else
                        {
                            ...
                            if (NPC.ai[1] == setupTime)
                            {
                                ...
                                for (int i = -3; i < 3; i++)
                                {
                                    ...
                                    Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center + new Vector2(255, 0).RotatedBy(rotation), new Vector2(-(i + 0.5f) * MathHelper.TwoPi / 6, 0), ProjectileType<Rework_EclipxieRayStar>(), ProjectileDamage, 0f, Main.myPlayer, ai0: 2, ai1: NPC.whoAmI);
                                }
                            }
                            if (NPC.ai[1] < setupTime + telegraphTime)
                            {
                ...
                case 10:
                    {
                        ...
                        else
                        {
                            ...
                            if ((NPC.ai[1] - setupTime) % 20 == 0 && NPC.ai[1] + 90 <= setupTime + attackTime)
                            {
                                ...
                                Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center, new Vector2(1, 0).RotatedByRandom(MathHelper.TwoPi), ProjectileType<Rework_EclipxieRaysBig>(), ProjectileDamage, 0f, Main.myPlayer, ai0: NPC.whoAmI, ai1: 4 + typeModifier);
                            }
                        }
                ...
                case 12:
                    {
                        ...
                        else
                        {
                            ...
                            if ((NPC.ai[1] - setupTime) % attackPeriod == 0)
                            {
                                ...
                                for (int i = -10; i <= 10; i++)
                                {
                                    Projectile.NewProjectile(NPC.GetSource_FromAI(), rayProducingNPC.Center + projectileOffset * i, projectileVelocity, ProjectileType<Rework_EclipxieRay>(), ProjectileDamage, 0f, Main.myPlayer, ai0: i, ai1: parity);
                                }
                            }
                        }
                ...
                case 13:
                    {
                        ...
                        else
                        {
                            ...
                            if ((NPC.ai[1] - setupTime) % 180 == 0 && NPC.ai[1] - setupTime < 360)
                            {
                                ...
                                for (int i = 0; i < 5; i++)
                                {
                                    Projectile.NewProjectile(NPC.GetSource_FromAI(), Main.npc[moonMinion].Center, new Vector2(32, 0).RotatedBy((player.Center - Main.npc[moonMinion].Center).ToRotation() + (i + 0.5f) * MathHelper.TwoPi / 5), ProjectileType<Rework_EclipxieRayStar>(), ProjectileDamage, 0f, Main.myPlayer, ai0: i == 0 ? 1.5f : 1, ai1: 1);
                                }
                            }
                            if ((NPC.ai[1] - setupTime - 90) % 180 == 0 && NPC.ai[1] - setupTime - 90 < 360)
                            {
                                ...
                                for (int i = 0; i < 5; i++)
                                {
                                    Projectile.NewProjectile(NPC.GetSource_FromAI(), Main.npc[sunMinion].Center, new Vector2(32, 0).RotatedBy((player.Center - Main.npc[sunMinion].Center).ToRotation() + (i + 0.5f) * MathHelper.TwoPi / 5), ProjectileType<Rework_EclipxieRayStar>(), ProjectileDamage, 0f, Main.myPlayer, ai0: i == 0 ? 1.5f : 1, ai1: 0);
                                }
                            }
                        }
                ...
                case 14:
                    {
                        ...
                        else if (NPC.ai[1] < setupTime + hoverTime)
                        {
                            if (NPC.ai[1] == setupTime)
                            {
                                Projectile.NewProjectile(NPC.GetSource_FromAI(), Main.npc[sunMinion].Center, new Vector2(0, NPC.ai[3]), ProjectileType<Rework_EclipxieRaysBig>(), ProjectileDamage, 0f, Main.myPlayer, ai0: Main.npc[sunMinion].whoAmI, ai1: 6f);
                                Projectile.NewProjectile(NPC.GetSource_FromAI(), Main.npc[moonMinion].Center, new Vector2(0, -NPC.ai[3]), ProjectileType<Rework_EclipxieRaysBig>(), ProjectileDamage, 0f, Main.myPlayer, ai0: Main.npc[moonMinion].whoAmI, ai1: 7f);
                            }
                ...
                case 15:
                    {
                        ...
                        else
                        {
                            ...
                            if ((NPC.ai[1] - setupTime) % 30 == 0 && NPC.ai[1] + 90 <= setupTime + attackTime)
                            {
                                ...
                                Projectile.NewProjectile(NPC.GetSource_FromAI(), Main.npc[sunMinion].Center, new Vector2(1, 0).RotatedByRandom(MathHelper.TwoPi), ProjectileType<Rework_EclipxieRaysBig>(), ProjectileDamage, 0f, Main.myPlayer, ai0: sunMinion, ai1: 4);
                                Projectile.NewProjectile(NPC.GetSource_FromAI(), Main.npc[moonMinion].Center, new Vector2(1, 0).RotatedByRandom(MathHelper.TwoPi), ProjectileType<Rework_EclipxieRaysBig>(), ProjectileDamage, 0f, Main.myPlayer, ai0: moonMinion, ai1: 5);
                            }
                        }
    ...
    [AutoloadBossHead]
    public class Rework_SolMoth : ModNPC
    {
        public override void SetStaticDefaults()
        {
            //group with other bosses
        ...
        public override void SetBestiary(BestiaryDatabase database, BestiaryEntry bestiaryEntry)
        {
            int associatedNPCType = NPCType<Rework_Eclipxie>();
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
        {
            ...
            if (RenderTargetLayer.IsActive<EclipxieTarget>())
            {
                ...
                Texture2D coronaTexture = Rework_Eclipxie.FireGradient.Value;
    ...
    [AutoloadBossHead]
    public class Rework_LunaButterfly : ModNPC
    {
        public override void SetStaticDefaults()
        {
            //group with other bosses
    ...
    public class Rework_EclipxieRay : ModProjectile
    {
    ...
    public class Rework_EclipxieOrbiter : ModProjectile
    {
    ...
    public class Rework_EclipxieRayStar : ModProjectile
    {
        public override string Texture => "Polarities/Textures/Glow58";
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            if (RenderTargetLayer.IsActive<ScreenWarpTarget>())
            {
                if (Projectile.timeLeft < rayTime)
                {
                    ...
                    {
                        Texture2D distortion = Rework_EclipxieRay.Distortion.Value;
                        //fully active
                        ...
                        if (drawCap)
                        {
                            Texture2D capTexture = Rework_EclipxieRaysBig.CapDistortion.Value;
            ...
            {
                Color color = Color.White;
                Texture2D texture = Projectile.ai[1] == 0 ? Rework_EclipxieRay.Solar.Value : TextureAssets.Projectile[ProjectileType<Rework_EclipxieRay>()].Value;
                ...
                for (int laserIndex = 0; laserIndex < projectileRays; laserIndex++)
                {
                    if (Projectile.timeLeft < rayTime)
                    {
                        ...
                        if (drawCap)
                        {
                            Texture2D capTexture = Rework_EclipxieRaysBig.CapSolar.Value;
                            Vector2 extraOffset = new Vector2(extra - (int)extra, 0).RotatedBy(Projectile.rotation + laserIndex * MathHelper.TwoPi / (int)Projectile.localAI[0]) * scale.X;
    ...
    public class Rework_EclipxieRaysBig : ModProjectile
    {
        ...
        public override string Texture => "Polarities/NPCs/Eclipxie/EclipxieRework/Rework_EclipxieRay";
        ...
        public override void AI()
        {
            ...
            switch ((int)Projectile.ai[1] / 2)
            {
                ...
                case 1:
                    ...
                    {
                        Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center + new Vector2((float)Math.Pow(Main.rand.NextFloat(), 2) * 2000, 0).RotatedBy(Projectile.rotation), owner.velocity + new Vector2(4, 0).RotatedBy(Projectile.rotation), ProjectileType<Rework_EclipxieMeteor>(), Rework_Eclipxie.ProjectileDamage, 0f, Projectile.owner, ai0: 2, ai1: Main.rand.Next(2));
                    }
                    break;
                case 2:
                    if (Projectile.timeLeft > rayTime)
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            else if (DrawLayer.IsActive<DrawLayerAdditiveBeforeScreenObstruction>())
            {
                ...
                {
                    Color color = Color.White;
                    Texture2D texture = (int)Projectile.ai[1] % 2 == 0 ? Rework_EclipxieRay.Solar.Value : TextureAssets.Projectile[ProjectileType<Rework_EclipxieRay>()].Value;
                    if (Projectile.timeLeft < rayTime)
                    {
                        const int numDraws = 18;
                        float segmentWidth = radius / numDraws;
    ...
    public class Rework_EclipxieMeteor : ModProjectile
    {
        public override string Texture => "Polarities/Textures/Glow58";
