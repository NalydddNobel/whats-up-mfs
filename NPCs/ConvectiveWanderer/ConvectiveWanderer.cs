...
namespace Polarities.NPCs.ConvectiveWanderer
{
    ...
    public class ConvectiveWanderer : ModNPC, IMultiHitboxSegmentUpdate
    {
        ...
        private const int HITBOXES_PER_TENTACLE = 16;
        private const int segmentsPerHitbox = 4;
        private const int specialSegmentsHeadMultiplier = 8;
        private const int specialSegmentsHead = 8 * specialSegmentsHeadMultiplier - 2;
        private const int hitboxSegmentOffset = 2;
        private const int segmentsTail = 8;
        private const int segmentsTailTendrils = 8;
        private const float segmentSeparation = 16f;

        private Vector2[] segmentPositions;
        private float[] segmentAngles;
        private float[] segmentPulseScaleAngles;
        private float tentacleAngleMultiplier = 0f;
        private float tentacleCompression = 1f;
        private float tentacleTiltAngle = 0f;
        private float tentacleCurveAmount = 0f;
        private float tendrilOutwardness = 0f;
        private float tendrilGlow = 0f;
        private float upDashTelegraphProgress = 0f;
        private float instadashTelegraphProgress = 0f;
        private float headSwingAlpha = 0f;
        private float deathAnimProgress = 0f;
        private float phase2TransitionProgress = 0f;

        private bool inPhase2 => phase2TransitionProgress >= 0.5f;
        ...
        private float tentacleAttacksHealthThreshold => Main.getGoodWorld ? 1f : Main.expertMode ? 0.8f : 0.75f;

        private float phase2HealthThreshold => Main.expertMode ? 0.6f : 0.5f; //we start in phase 2 in ftw //TODO: Shift health thresholds earlier
        ...
        public override void AI()
        {
            ...
            switch (NPC.ai[0])
            {
                ...
                case 2:
                    {
                        ...
                        const float dashStartVelocity = 109.545f;
                ...
                case 6:
                    {
                        ...
                        if (NPC.ai[1] + 30 < startSetupTime + totalAttackTime * attackRepetitions && (int)(NPC.ai[1] + 30 - startSetupTime) % totalAttackTime < setupTime)
                        {
                            ...
                            int numParticles = inPhase2 ? Main.rand.Next(1, 3) : 1;
                ...
                case 9:
                    {
                        const int setupTime = 180;
                        const int attackTime = 60;
                        const int windDownTime = 60;

                        int side = Vector2.Dot(NPC.Center - player.Center, new Vector2(0, -1).RotatedBy(NPC.rotation)) > 0 ? 1 : -1;

                        //straighten out rapidly
                        if (NPC.ai[1] < setupTime + attackTime)
                        {
                            rotationAmount *= 1 + NPC.ai[1] / 30f;

                            if (NPC.ai[1] > 120)
                            {
                                instadashTelegraphProgress = (NPC.ai[1] - 120) / 120;
                            }
                        }

                        if (NPC.ai[1] < setupTime)
                        {
                            //move into setup position
                            float timeLeft = setupTime - NPC.ai[1];

                            Vector2 goalPosition = player.Center + (NPC.Center - player.Center).SafeNormalize(Vector2.Zero) * Math.Min(400f, (NPC.Center - player.Center).Length());
                            Vector2 goalVelocity = (goalPosition - NPC.Center) * Math.Min(timeLeft, 60) / 60f / Math.Max(timeLeft, 60f);
                            NPC.velocity += (goalVelocity - NPC.velocity) / Math.Min(Math.Max(1f, timeLeft - 60f), 60f);

                            if (timeLeft < 120)
                            {
                                float maxTurn = (2 * MathHelper.TwoPi / 120f) * (1 - timeLeft / 120);
                                NPC.rotation += side * maxTurn;
                                int newSide = Vector2.Dot(NPC.Center - player.Center, new Vector2(0, -1).RotatedBy(NPC.rotation)) > 0 ? 1 : -1;
                                if (newSide != side) NPC.rotation = (player.Center - NPC.Center).ToRotation();
                        ...
                        {
                            //rotate towards player while telegraphing dash
                            float timeLeft = setupTime + attackTime - NPC.ai[1];

                            if (NPC.ai[1] == setupTime) NPC.ai[2] = side;

                            NPC.velocity = new Vector2(32 / timeLeft, 0).RotatedBy(NPC.rotation);

                            float maxTurn = (2 * MathHelper.TwoPi / 120f) * (float)Math.Pow(timeLeft / attackTime, 4);
                            NPC.rotation += side * maxTurn;
                            int newSide = Vector2.Dot(NPC.Center - player.Center, new Vector2(0, -1).RotatedBy(NPC.rotation)) >= 0 ? 1 : -1;
                            if (newSide != side) NPC.rotation = (player.Center - NPC.Center).ToRotation();
                            useDefaultRotation = false;

                            angleSpeed = NPC.ai[2] * 0.2f * (1 - timeLeft / attackTime);
                            tentacleAngleMultiplier += (NPC.ai[2] * 0.1f - tentacleAngleMultiplier) / 10f;
                        }
                        else
                        {
                            ...
                            {
                                //instadash
                                Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center, new Vector2(1, 0).RotatedBy(NPC.rotation), ProjectileType<ConvectiveWandererLingeringDeathray>(), 16, 2f, Main.myPlayer, ai0: NPC.whoAmI);
                                ...
                                NPC.position = NPC.position + new Vector2(4000f + Vector2.Dot(player.Center - NPC.Center, new Vector2(1, 0).RotatedBy(NPC.rotation)), 0).RotatedBy(NPC.rotation);
                                rotationAmount = 1000f;
                            }
                            else
                            {
                                Idle();
            ...
            {
                float amountLeft = 1 / phase2HealthThreshold * NPC.life / NPC.lifeMax;
        ...
        private PriorityQueue<DrawData, float> drawDatas;
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
        {
            ...
            if (DrawLayer.IsActive<DrawLayerAdditiveAfterLiquids>())
            {
                ...
                if (instadashTelegraphProgress > 0)
                {
                    Rectangle frame = StarConstruct.StarConstruct.DashTexture.Value.Frame();
                    Vector2 origin = new Vector2(StarConstruct.StarConstruct.DashTexture.Value.Width, StarConstruct.StarConstruct.DashTexture.Value.Height / 2);

                    float scale = 8 * (1 - instadashTelegraphProgress);

                    spriteBatch.Draw(StarConstruct.StarConstruct.DashTexture.Value, NPC.Center + new Vector2(280, 0).RotatedBy(NPC.rotation) - screenPos, frame, ModUtils.ConvectiveFlameColor((float)Math.Pow(instadashTelegraphProgress * 0.5f + 0.5f, 2)) * (0.5f / scale), NPC.rotation + MathHelper.Pi, origin, new Vector2(16, scale), SpriteEffects.None, 0f);
                }
            }
            else
            {
        ...
        private void DrawHeadSwingPrimTrail()
        {
        ...
        private const int BASE_TEXTURE_HEIGHT = 128;
        private const int SIDES_PER_SEGMENT = 2;
        private const int DRAWS_PER_SIDE = 4;
        private const int DRAWS_PER_SEGMENT = SIDES_PER_SEGMENT * DRAWS_PER_SIDE;
        ...
        private Vector2 BaseSegmentPosition(int index)
        {
            if (index < 0)
            {
        ...
        private float PulseScale(int index)
        {
        ...
        private float TrueBaseScale(int index)
        {
        ...
        private float BaseScale(int index)
        {
        ...
        private int TendrilIndex(int index)
        {
        ...
        private void DrawSegment(PriorityQueue<DrawData, float> drawDatas, SpriteBatch spriteBatch, Vector2 screenPos, int index, Color color, float alphaMultiplier)
        {
        ...
        private const int TENTACLE_SEGMENTS = 32;
        private const int TENTACLE_HEAD_SEGMENTS = 8;
        private const int TENTACLE_SEGMENT_SEPARATION = 8;

        public const int TENTACLE_ATTACH_SEGMENT_INDEX = -4;

        private void DrawTentacle(PriorityQueue<DrawData, float> drawDatas, SpriteBatch spriteBatch, Vector2 screenPos, float angleOffset, Color color)
        {
        ...
        private const int BASE_TENTACLE_TEXTURE_HEIGHT = 96;
        private const int SIDES_PER_TENTACLE_SEGMENT = 1;
        private const int DRAWS_PER_TENTACLE_SIDE = 12;
        private const int DRAWS_PER_TENTACLE_SEGMENT = SIDES_PER_TENTACLE_SEGMENT * DRAWS_PER_TENTACLE_SIDE;
        private const float TENTACLE_HEAD_SEPARATION_SCALE_MULT = 4;
        private const float TENTACLE_POSITION_INDEX_OFFSET = TENTACLE_HEAD_SEGMENTS / TENTACLE_HEAD_SEPARATION_SCALE_MULT + 2;
        private const float EFFECTIVE_TENTACLE_SEGMENTS = TENTACLE_SEGMENTS + TENTACLE_POSITION_INDEX_OFFSET;

        //a whole bunch of tentacle-related methods
        private int TentacleSegmentWidth(float index)
        {
            return (index > 0) ? TENTACLE_SEGMENT_SEPARATION : (int)(TENTACLE_SEGMENT_SEPARATION / TENTACLE_HEAD_SEPARATION_SCALE_MULT);
        }

        private Vector2 TentacleEffectiveBasePosition(float index, float baseAngle, float baseRotation, float baseRadius, Vector2 originalBasePosition)
        {
        ...
        private float TentacleBaseAngleOffset(float index)
        {
            return (index <= 0) ? 0 :
                index * tentacleAngleMultiplier;
        }

        private float TentacleRotation(float index)
        {
            return tentacleTiltAngle + tentacleCurveAmount * index / EFFECTIVE_TENTACLE_SEGMENTS;
        }

        private Vector2 TentacleRadialOffset(float index, float baseRadius)
        {
        ...
        private Vector2 TentacleOffsetUsingAngle(float index, float baseAngle, float baseRadius)
        {
        ...
        private float TentacleRadiusMult(float index)
        {
            return (index >= 0) ?
        ...
        private void DrawTentacleSegment(PriorityQueue<DrawData, float> drawDatas, SpriteBatch spriteBatch, Vector2 screenPos, float baseAngle, float baseRotation, Vector2 originalBasePosition, float baseRadius, int index, Color color)
        {
    ...
    public class ConvectiveWandererAcceleratingShot : ModProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            //phase 2 versions fade to darker blue
            if (Projectile.timeLeft < 45 && Projectile.ai[1] != 0)
            {
                drawColor = new Color((int)(drawColor.R * Projectile.timeLeft / 45f), (int)(drawColor.G * Projectile.timeLeft / 45f), drawColor.B, drawColor.A);
            }
    ...
    public class ConvectiveWandererHeatVortex : ModProjectile
    {
        ...
        private void MakeParticle(float minSpeed = 12f, float maxSpeed = 20f)
        {
    ...
    public class ConvectiveWandererFlamethrower : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            Projectile.Center = owner.GetGlobalNPC<MultiHitboxNPC>().preModifyDataCenter + 112 * Projectile.velocity;
    ...
    public class ConvectiveWandererLingeringDeathray : ModProjectile
    {
        ...
        public override void OnSpawn(IEntitySource source)
        {
            ...
            {
                if (Main.projectile[i].active && Main.projectile[i].type == Type && Main.projectile[i].ai[0] == Projectile.ai[0] && Main.projectile[i] != Projectile)
                {
                    if (Main.projectile[i].timeLeft > 30) Main.projectile[i].timeLeft = Math.Min(30, (3600 - Main.projectile[i].timeLeft) * 10);
                }
            }
        }
        ...
        public override void AI()
        {
            if (Projectile.timeLeft > 30 && Projectile.timeLeft < 3597) Projectile.timeLeft = 3597;

            NPC owner = Main.npc[(int)Projectile.ai[0]];
            if ((!owner.active || owner.ai[0] == -3) && Projectile.timeLeft > 30)
            {
                Projectile.timeLeft = Math.Min(30, (3600 - Projectile.timeLeft) * 10);
            }

            Projectile.ai[1]++;

            if (Projectile.timeLeft < 30) Projectile.scale = 128 * Projectile.timeLeft / 30f;
            if (Projectile.timeLeft >= 3597) Projectile.scale = 128 * (3600f - Projectile.timeLeft) / 3f;

            Projectile.scale *= 1 + 0.1f * (float)Math.Sin(Projectile.ai[1] * MathHelper.TwoPi / 40f);
        ...
        public override bool? Colliding(Rectangle projHitbox, Rectangle targetHitbox)
        {
            ...
            float point = 0;
            return Collision.CheckAABBvLineCollision(targetHitbox.TopLeft(), targetHitbox.Size(), Projectile.Center - new Vector2(length / 2, 0).RotatedBy(Projectile.rotation), Projectile.Center + new Vector2(length / 2, 0).RotatedBy(Projectile.rotation), Projectile.scale / 3, ref point);
        }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Rectangle lineFrame = new Rectangle(128, 0, 1, 256);
            Vector2 lineCenter = lineFrame.Size() / 2;

            float colorPulse = (3 + (float)Math.Sin(Projectile.ai[1] * MathHelper.TwoPi / 60f)) / 3;
            Color drawColor = ModUtils.ConvectiveFlameColor((float)Math.Pow(0.5f * colorPulse + 0.5f, 2));
